...
export const usePeriodDependency = (options: UsePeriodDependencyOptions) => {
  ...
  // ✅ FIXED: Check if missed first, then check if waiting for teacher
  const isWaitingForTeacher = 
    data?.status !== 'MISSED' && // ← Don't show "waiting" if missed!
    data?.incompleteReason !== 'MISSED_GRACE_PERIOD' && // ← Check incomplete reason
  ...
  const statusMessage = useCallback(() => {
    ...
    // ✅ NEW: Check for missed assessments first
    if (data.status === 'MISSED' || data.incompleteReason === 'MISSED_GRACE_PERIOD') {
      return 'Assessment missed - deadline expired';
    }
