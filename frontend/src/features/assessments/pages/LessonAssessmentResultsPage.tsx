...
interface SubmissionAnswer {
  ...
  isCorrect: boolean;
  marksObtained: number | null; // ✅ Can be null for ungraded questions
  maxMarks: number;
  teacherFeedback?: string;
}

interface LessonAssessmentSubmission {
  id: number;
  assessmentId: number;
...
const LessonAssessmentResultsPage: React.FC = () => {
  const { submissionId } = useParams<{ submissionId: string }>();
  const navigate = useNavigate();

  // Fetch submission results
  const { data: submission, isLoading, error } = useQuery<LessonAssessmentSubmission>({
    queryKey: ['lessonAssessmentSubmission', submissionId],
    queryFn: async () => {
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="space-y-6">
          {submission.answers.map((answer, index) => {
            // ✅ Handle null marksObtained for ungraded questions
...
export default LessonAssessmentResultsPage;