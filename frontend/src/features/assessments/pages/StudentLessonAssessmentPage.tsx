...
const StudentLessonAssessmentPage: React.FC = () => {
  ...
  // Fetch lesson topic
  const { data: topic, isLoading: loadingTopic } = useQuery<LessonTopic>({
  ...
  const { data: existingSubmission } = useCheckSubmission(
    Number(lessonTopicId), 
    studentProfileId
  );
  ...
  } = useAssessmentByLesson(Number(lessonTopicId), studentProfileId);

  // Get assessment questions
  const { 
    data: questions = [], 
    isLoading: loadingQuestions 
  } = useAssessmentQuestions(assessment?.id || 0, false);

  // Submit mutation
  const submitMutation = useSubmitLessonAssessment();

  // ✅ FIXED: Check assessment access
  const {
    ...
    assessment?.id || 0,
    studentProfileId || 0,
    60000 // Update every minute for lesson assessments
  );

  // ✅ FIXED: Start polling service with correct function signature
  useEffect(() => {
    if (!studentProfileId || !assessment?.id) return;
    ...
    assessmentAccessService.startPolling(
      ...
      // ✅ FIXED: Return the promise result, not the entire query object
      async () => {
    ...
    return () => {
      assessmentAccessService.stopPolling(assessment.id, studentProfileId);
    };
  }, [assessment?.id, studentProfileId]);
  ...
  // Loading states
  if (loadingTopic || loadingAssessment || loadingQuestions || loadingProfile) {
    return (
  ...
  if (!topic) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
          <p className="text-red-600 font-medium">Lesson topic not found</p>
          <button
            onClick={() => navigate(-1)}
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
          <p className="text-yellow-800 font-medium">No assessment available for this lesson yet</p>
          <button
            onClick={() => navigate(-1)}
            className="mt-4 text-blue-600 hover:underline"
          >
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        ...
        {/* ✅ FIXED: Time remaining banner with null check */}
          ...
          <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
            <p className="text-sm text-blue-800 flex items-center gap-2">
              ...
              <span>
                ...
                and this lesson will be marked as complete.
              </span>
            </p>
          </div>
        </div>
...
export default StudentLessonAssessmentPage;