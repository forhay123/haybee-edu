// frontend/src/features/individual/pages/admin/AdminViewTeacherSchedulePage.tsx
...
const AdminViewTeacherSchedulePage: React.FC = () => {
  ...
  const { data: teacher, isLoading: teacherLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return response.data.find((t: any) => t.id === parseInt(teacherId!));
    },
  ...
  const { data: teacherSubjects = [] } = useQuery({
    queryKey: ["admin-teacher-subjects", teacherId],
    queryFn: async () => {
      if (!teacher) return [];
  ...
  const teacherSubjectIds = useMemo(() => {
    return new Set(teacherSubjects.map((s: any) => s.id));
  }, [teacherSubjects]);

  // Fetch all INDIVIDUAL students' timetables
  const { data: allTimetables = [] } = useQuery({
    queryKey: ["admin-all-individual-timetables"],
    queryFn: async () => {
      const response = await axios.get("/individual/timetable/admin/all");
      return response.data.filter((t: any) => t.processingStatus === "COMPLETED");
    },
  });

  // Generate teacher's schedule from all students
  const { data: scheduleData = [], isLoading, error } = useQuery({
    queryKey: ["admin-teacher-schedule", teacherId, teacherSubjectIds.size],
    queryFn: async () => {
      if (teacherSubjectIds.size === 0) return [];
        ...
        try {
          ...
          weeklySchedule.forEach((daySchedule) => {
            daySchedule.periods.forEach((period) => {
              // Only include if teacher teaches this subject
              if (teacherSubjectIds.has(period.subjectId)) {
                allSchedules.push({
    ...
    enabled: Boolean(teacherSubjectIds.size > 0 && allTimetables.length > 0),
    staleTime: 5 * 60 * 1000,
  });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">No teacher ID provided.</p>
          <button
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading teacher schedule...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-8 max-w-md text-center">
          ...
          <p className="text-yellow-700 mb-4">
            {teacher.userName} doesn't have any subjects assigned yet.
          </p>
          <button
            onClick={() => navigate("/admin/individual/teacher-schedules")}
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            {teacher.userName}'s Teaching Schedule
          </h1>
          <p className="text-gray-600">
            View teaching schedule for {teacher.userName}'s assigned subjects
          </p>
        </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
            ...
            <p className="text-gray-600">
              {selectedDay !== "ALL" || selectedSubject !== "ALL"
                ...
                : `${teacher.userName} doesn't have any scheduled classes for their subjects yet`}
            </p>
          </div>
        )}
      </div>
...
export default AdminViewTeacherSchedulePage;